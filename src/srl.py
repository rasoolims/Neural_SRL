...
class SRLLSTM:
    def __init__(self, words, lemmas, pos, roles, w2i, pl2i, options):
        ...
        self.inp_dim = self.d_w + self.d_pos + (self.edim if self.external_embedding is not None else 0)
        ...
        self.arg_h = self.model.add_parameters((self.d_h*2, self.d_h*2))
        self.pred_h = self.model.add_parameters((self.d_h*2, self.d_h*2 + self.d_l))
    ...
    def getBilstmFeatures(self, sentence, train):
        embeds = self.getEmbeddings(sentence, train)
        return self.deep_lstms.transduce(embeds)

    def buildGraph(self, sentence):
        errs = []
        arg_h = parameter(self.arg_h)
        pred_h = parameter(self.pred_h)
        bilstms = self.getBilstmFeatures(sentence.entries, True)
        as_args = [rectify(arg_h*bilstms[i]) for i in range(len(bilstms))]
        ...
        for p in xrange(len(sentence.predicates)):
            ...
            v_p = rectify(pred_h*concatenate([bilstms[pred_index], self.x_le[pred_lemma_index]]))
