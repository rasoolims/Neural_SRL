...
class SRLLSTM:
    def __init__(self, words, pos, roles, w2i, pl2i, possible_args, chars, l2i, options):
        self.model = Model()
        ...
        self.lemmas = {l: ind + 2 for l, ind in l2i.iteritems()}
        ...
        self.use_all_lemma = options.use_all_lemma
    ...
    def getBilstmFeatures(self, sentence, train):
        ...
        for token in sentence:
            ...
            x_le.append(lookup(self.x_le, int(self.lemmas.get(token.lemma, 0)) if not word_drop else 0)) if (token.is_pred or self.use_all_lemma) else x_le.append(self.lemmas.get(token.lemma, 1))
    ...
    def buildGraph(self, sentence, correct):
        ...
        for p in xrange(len(sentence.predicates)):
            ...
            pred_lemma_index = 0 if word_drop else self.pred_lemmas[sentence.entries[pred_index].lemma]
